...
- Machine Learning Algorithms:
  ...
  * Natural Language Pocessing - import nltk (the library to process text mining problems)
    1. ```shell import string to remove punctuation ```; ```shell from nltk.corpus import stopwords ``` to use stopwords.words('english') to eliminate the english stopwords.
    2. ```shell from sklearn.feature_extraction.text import CountVectorizer ```, this is to calculate the words of bag, simply use fit and transform to the data. .nnz is to find non zero values, and .shape is to know the words and how many sentences you have
    3. ```shell from sklearn.feature_extraction.text import TfidfVectorizer ```, this is to calculate the tf-idf value, also fit and transform the data. .volcabulary_ is to see a specific word's tf-idf value
    4. ```shell from sklearn.naive_bayes import MultinomialNB ``` as the model. simply fit and predict. Noted that there're plenty other models you can choose from
    5. ```shell from sklearn.pipeline import Pipeline ```, this can collect all the previous steps into a list of tuples solution bags. A great way to summarize the methods you used. 
- Deep Learning Algorithms:
  ...
  * Tensorflow and keras - ```shell from tensorflow.keras.models import Sequential ```; ```shell from tensorflow.keras.layers import Dense, Activation ```; ```shell from tensorflow.keras.optimizers import Adam ```
- Big Data and Spark
  1. AWS account setup
